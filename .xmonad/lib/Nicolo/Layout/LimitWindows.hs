...
module Nicolo.Layout.LimitWindows (
    -- * Usage
    -- $usage
...
increaseLimit :: X ()
increaseLimit = do
    windowsCount <- length . W.index . windowset <$> get
    sendMessage . LimitChange $ min windowsCount . succ
...
instance LayoutModifier LimitWindows a where
     ...
     modifyDescription (LimitWindows _ n) _ = toLetter n ++ " Window" ++ (if n > 1 then "s" else "")
         where digitsToLetters = M.fromList [(1, "One"), (2, "Two"), (3, "Three"), (4, "Four"), (5, "Five")]
               toLetter x = fromMaybe (show x) $ M.lookup x digitsToLetters
