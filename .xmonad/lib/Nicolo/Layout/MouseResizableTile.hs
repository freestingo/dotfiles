...
----------------------------------------------------------------------------
-- |
-- Module      :  Nicolo.Layout.MouseResizableTile
-- Copyright   :  (c) Nicolo Traini 2021
...
module Nicolo.Layout.MouseResizableTile (
                                    ...
                                    MRTMessage (ShrinkSlave, ExpandSlave, ModifyDraggerGap, ToggleDraggerGap),
...
data DraggerType = FixedDragger
                    ...
                    , isActive :: Bool          -- ^ when false, ignore width settings and just
                                                --   set the dragger on window borders
                    }
                    deriving (Show, Read)
...
mouseResizableTile = MRT 1 0.5 0.5 0.03 [] [] [] (FixedDragger 6 6 True) 0 0 False
...
instance LayoutClass MouseResizableTile Window where
    ...
    handleMessage st m
        ...
        | Just (ModifyDraggerGap amount) <- fromMessage m =
            return $ Just $ st { draggerType = updateDragger (draggerType st) amount }
        | Just ToggleDraggerGap <- fromMessage m =
            return $ Just $ st { draggerType = toggleDragger (draggerType st) }
        ...
        where releaseResources = mapM_ deleteDragger $ draggers st
              updateDragger (FixedDragger gap drag isActive) amount = FixedDragger (max 0 $ gap + amount) (max 0 $ drag + amount) isActive
              toggleDragger (FixedDragger gap drag isActive) = FixedDragger gap drag (not isActive)
...
draggerGeometry (FixedDragger g d isActive)
    | isActive  = return (fromIntegral $ g `div` 2, g, fromIntegral $ d `div` 2, d)
    | otherwise = do
