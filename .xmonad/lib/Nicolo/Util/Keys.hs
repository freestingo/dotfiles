...
myKeysEZConfig conf = mkKeymap conf [
    ...
    , ("M-S-C-k", swapNextScreen)
    -- Swap current screen with previous screen
    , ("M-S-C-j", swapPrevScreen)
    -- Quit xmonad (logout)
    , ("M-S-q", confirmLogout)
    -- Restart xmonad
    ...
    -- Move the mouse cursor to the current screen's lower right corner
    ...
    -- Search xmonad-contrib modules
    , ("M-C-x", searchXMonadContrib)
    ...
    , ("<Print>", takeScreenshot $ Scrot AllDisplays defScrotName defScrotCmd)
    -- Take screenshot of selected area
    , ("M-<Print>", takeScreenshot $ Scrot SelectedArea defScrotName defScrotCmd)
    -- Take screenshot of currently focused window
    -- TODO find out why this doesn't work for some reason
    -- , ("M-S-<Print>", takeScreenshot FocusedWindow)
